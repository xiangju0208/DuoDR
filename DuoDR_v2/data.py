...
# Get the absolute path of the directory containing this file (data.py)
BASE_DIR = os.path.dirname(os.path.abspath(__file__))

_paths = {
    'Gdataset': os.path.join(BASE_DIR, 'raw_data/drug_data/Gdataset/Gdataset.mat'),
    'Cdataset': os.path.join(BASE_DIR, 'raw_data/drug_data/Cdataset/Cdataset.mat'),
    'Ldataset': os.path.join(BASE_DIR, 'raw_data/drug_data/Ldataset/lagcn'),
    'lrssl': os.path.join(BASE_DIR, 'raw_data/drug_data/lrssl'),
}
...
class DrugNovoLoader(object):
    ...
    def _load_drug_data(self, file_path, data_name):
        ...
        if data_name in ['Gdataset', 'Cdataset']:
            ...
            # [VERIFIED ALIGNMENT]
            # drug_dis.txt columns are Disease Names (e.g., 'AIDS-Related Opportunistic Infections')
            # dis_sim.txt index are MeSH IDs (e.g., 'D017088')
            # Although the labels differ, the order corresponds exactly (i-th column name corresponds to i-th row ID).
            # Verified via MeSH mapping check on first 3 elements:
            # 0: 'AIDS-Related Opportunistic Infections' <-> 'D017088'
            # 1: 'Abdominal Abscess' <-> 'D018784'
            # 2: 'Abnormalities, Drug-Induced' <-> 'D000014'
...
class DrugDataLoader(object):
    ...
    def _load_drug_data(self, file_path, data_name):
        ...
        # Use logic from AdaDR-main (2) (Separate Pos/Neg Split)
...
class DrugColdStartDataLoader(DrugDataLoader):
    def _load_drug_data(self, file_path, data_name):
        association_matrix = None
        if data_name in ['Gdataset', 'Cdataset']:
            data = sio.loadmat(file_path)
        ...
        drug_indices = np.arange(self._num_drug)
        kfold = KFold(n_splits=10, shuffle=True, random_state=1024)

        cv_data = {}
        for cv_num, (train_drug_idx, test_drug_idx) in enumerate(kfold.split(drug_indices)):
            train_drugs = drug_indices[train_drug_idx]
            test_drugs = drug_indices[test_drug_idx]

            train_sub = association_matrix[train_drugs, :]
            pos_local_row, pos_col = np.nonzero(train_sub == 1)
            pos_row = train_drugs[pos_local_row]

            num_pos = len(pos_row)
            pos_values = np.ones(num_pos, dtype=np.float32)

            neg_local_row, neg_col = np.nonzero(train_sub == 0)
            neg_row = train_drugs[neg_local_row]

            neg_sample_size = num_pos * 10
            if len(neg_row) > neg_sample_size and neg_sample_size > 0:
                rng = np.random.default_rng(1024 + cv_num)
                sampled_idx = rng.choice(len(neg_row), size=neg_sample_size, replace=False)
                neg_row = neg_row[sampled_idx]
                neg_col = neg_col[sampled_idx]

            neg_values = np.zeros(len(neg_row), dtype=np.float32)

            train_edge_drug = np.concatenate([pos_row, neg_row])
            train_edge_dis = np.concatenate([pos_col, neg_col])
            train_values = np.concatenate([pos_values, neg_values])

            train_data_info = pd.DataFrame({
                'drug_id': train_edge_drug,
                'disease_id': train_edge_dis,
                'values': train_values
            })

            test_edge_drug = np.repeat(test_drugs, self._num_disease)
            test_edge_dis = np.tile(np.arange(self._num_disease), len(test_drugs))
            test_values = association_matrix[test_edge_drug, test_edge_dis].astype(np.float32)

            test_data_info = pd.DataFrame({
                'drug_id': test_edge_drug,
                'disease_id': test_edge_dis,
                'values': test_values
            })
